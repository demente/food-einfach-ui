...
<div class="container">
  ...
  <div class="form-group" v-for="ingredient in recipe.ingredients" v-bind:key="ingredient.id">
        ...
        <FoodAutocompletionField :food="ingredient.food" @updateFood="ingredient.food = $event"/>
...
export default {
  ...
  created() {
    ...
    saveRecipe() {
      axios.post("http://localhost:8080/recipes/", this.recipe).then((resp) => this.showSaveSuccess()).catch(() => this.showSaveFailure());
    },
    addIngredient(){
